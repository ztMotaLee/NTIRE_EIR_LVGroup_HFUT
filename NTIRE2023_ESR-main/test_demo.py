...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        from models.team01_EDRN import EDRN
        name, data_range = f"{model_id:02}_EDRN", 255.0
        model_path = os.path.join('model_zoo', 'team01_edrn.pth')
        model = EDRN()
        model.load_state_dict(torch.load(model_path), strict=True)
    else:
...
def select_dataset(data_dir, mode):
    if mode == "test":
        ...
        path = [
            (
                os.path.join(data_dir, f"DIV2K_valid_LR/X4/{i:04}x4.png"),
